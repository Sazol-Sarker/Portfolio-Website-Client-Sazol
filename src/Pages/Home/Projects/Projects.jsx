      ...
      <div id="projects" className="my-10">
        ...
        <div className="flex flex-col items-center">
          {projects.map((project, idx) => {
            return (
              <div key={idx} className="w-full max-w-4xl text-white border-2 my-5 rounded-md hover:bg-slate-700/20  p-5">
                <div className="flex flex-col md:flex-row justify-between items-center">
                 
                    <div onClick={()=>window.open(project?.links?.live,"_blank")}
                ...
                <p  className="mt-5 text-md md:text-lg">{project.description}</p>
                <ul className="list-disc ml-2 pt-5">
                  {project.features.map((feature, idx) => (
                    <li key={idx} className="text-md md:text-lg">{feature}</li>
                  ))}
                </ul>
                {/* technologies badge */}
                <div className="flex flex-wrap mt-5 mb-3 gap-2">
                  {project.technologies.map((tech, idx) => {
                    return (
                      <div key={idx} className="text-sm md:text-md text-white bg-gray-700 px-2 py-1 rounded-xl">
                        {tech.title}
                      </div>
                    );
                  })}
